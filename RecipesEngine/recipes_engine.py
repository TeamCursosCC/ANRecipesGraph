...
class RecipesEngine:
	...
	def suggest_recipes_from_ingredients(self, original_ingredients: list):
		...
		for ingredients in combinaciones:
			...
			subgrafos_ego = [nx.ego_graph(self.recipesGraph_undirected, n=nodo, radius=1) for nodo in ingredients]	
			
			#se hace la interseccion entre los nodos de todos los grafos ego 
			interseccion = set.intersection(*[set(subgrafo.nodes()) for subgrafo in subgrafos_ego])	
